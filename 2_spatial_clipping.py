...
red_lim, faint_lim, tip = np.zeros(len(info)), np.zeros(len(info)), np.zeros(len(info))
for i in range(len(info)):
    field = info['field'][i]
    print(field)
    data = pd.read_csv('csv/{:s}.csv'.format(field))
    color, mag = data['MAG1_ACS']-data['MAG2_ACS'], data['MAG2_ACS']
    band = color_band(color, mag, slope_range)
    slope[i], inter[i] = band[0], band[1]
    red_lim[i] = 0.3 + info['A606'][i] - info['A814'][i]
    
    mini = np.round(np.min(mag)*10)/10
    maxi = np.ceil(np.max(mag)*10)/10+0.1
    ...
    faint_lim[i] = maglist[np.abs(err-0.1)==mini][0]
...
while i < len(info):
    ...
    for j in range(len(subinfo)):
        ...
        ax[j].plot([-1, 3.5], [subinfo['faint_lim'][j]]*2, 'g-', lw=2)
        ax[j].plot([subinfo['red_lim'][j]]*2, [35, 20], 'r-', lw=2)
        ax[j].plot([-1, 3.5], [subinfo['tip'][j]]*2, 'k-', lw=1)
        ...
        ax[j].set_xlim(-1, 3.5)
        blim = np.round(subinfo['faint_lim'][j]*2)/2 + 1
        ax[j].set_ylim(blim, blim-6)
        ax[j].set_title(subinfo['field'][j])
    plt.savefig('cmd/{:s}_v{:d}.png'.format(gal,version))
    plt.show()
...
for i in range(len(info)):
    ...
    plt.savefig('clipping_map/{:s}_v{:d}.png'.format(info['field'][i],version))
    plt.show()
    
    data_clip.to_csv('clipped_csv/{:s}_v{:d}.csv'.format(info['field'][i],version), index=False)
    slope[i], inter[i] = color_band(color, mag, slope_range)

info['slope_ac'], info['inter_ac'] = slope, inter
info = info.drop(['tip'], axis = 1)
info.to_csv('info/ghosts_info_v{:d}.csv'.format(version), index=False)
...
while i < len(info):
    ...
    fig, ax = plt.subplots(1, len(subinfo), figsize = (3.5*len(subinfo), 6))
    for j in range(len(subinfo)):
        data = pd.read_csv('clipped_csv/{:s}_v{:d}.csv'.format(subinfo['field'][j],version))
        color, mag = data['MAG1_ACS']-data['MAG2_ACS'], data['MAG2_ACS']
        ax[j].plot(color, mag, 'k.', ms=1)
        ...
        ax[j].set_ylim(blim, blim-6)
        ax[j].set_title(subinfo['field'][j])
    plt.savefig('clipped_cmd/{:s}_v{:d}.png'.format(gal,version))
    plt.show()
