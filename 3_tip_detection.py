    ...
    if len(peakq) > 0:
        ...
        for i in range(len(pos)):
            if (peak[i] >= peak_min * maxv) & (fwhm[i] > fwhm_min):
                ...
                fwhm_good.append(fwhm[i]/100)
    trgb = binval[pos_good]
    return trgb, np.array(ratio_good), np.array(nbt_good), np.array(fwhm_good)

#change version here
version = 4.5

vinfo = pd.read_csv('version_code3.csv')
vinfo = vinfo[vinfo['version']==version].reset_index(drop=True)
info = pd.read_csv('info/ghosts_info_v{:d}.csv'.format(int(np.floor(version))))
ratio_min, nbt_min, peak_min = 3, 0, 0.6
cband, smooth, weighting = vinfo['band'][0], vinfo['smoothing'][0], vinfo['weighting'][0]
...
for i in range(len(info)):
    print(info['field'][i])
    data = pd.read_csv('clipped_csv/{:s}_v{:d}.csv'.format(info['field'][i],int(np.floor(version))))
    ...
    trgb, ratio, nbt, fwhm = findpeak(binval, edres, mag, ratio_min, nbt_min, peak_min)
    judge = (ratio >= vinfo['ratio_cut'][0]) & (nbt >= vinfo['nbt_cut'][0])
    ...
    agg_fwhm = np.append(agg_fwhm, fwhm)
...
data['A606'] = agg_a606
data['A814'] = agg_a814
data.to_csv('detection/ghosts_detection_v{:s}.csv'.format(str(version)), index = False)
